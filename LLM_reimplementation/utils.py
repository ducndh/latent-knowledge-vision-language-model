...
def get_parser():
    ...
    parser.add_argument("--prompt_name", nargs='+', default="imdb", help="Name of the dataset to use")
    ...
    parser.add_argument("--text_key", type=str, default="text", help="Name of the text")
    parser.add_argument("--label_key", type=str, default="label", help="Name of the label")
    
    # args for evaluation
    parser.add_argument("--models", nargs='+', default="imdb", help="Name of the models to use")
    parser.add_argument("--datasets", nargs='+', default="imdb", help="Name of the datasets to use")
    parser.add_argument("--splits", nargs='+', default="imdb", help="Name of the models to use")
...
def load_single_generation(args, generation_type="hidden_states"):
    ...
    arg_dict = vars(args)
    exclude_keys = ["save_dir", "cache_dir", "device" ,"prompt_name"]
    filename = generation_type + "__" + "__".join(['{}_{}'.format(k, v) for k, v in arg_dict.items() if k not in exclude_keys]) + ".npy".format(generation_type)
    filename = filename.replace('/','_')
    if len(filename)>150:
        filename=filename[:150]+'all_layers_{}'.format(args.all_layers)
...
class ContrastDataset(Dataset):
    ...
    def __init__(self, raw_dataset, tokenizer, all_prompts, prompt_idx, 
                 model_type="encoder_decoder", use_decoder=False, device="cuda",text_key='text',label_key='label'):
        ...
        #self.num_labels=len(self.raw_dataset.features["label"].names)
    ...
    def __getitem__(self, index):
        ...
        text, true_answer = data, data['label']
        label_list = self.prompt.get_answer_choices_list(data)
        lst = list(filter(lambda x: x != true_answer, list(range(len(label_list)))))
        wrong_answer=np.random.choice(lst)
        answer=np.array([wrong_answer,true_answer])
        neg_example=deepcopy(text)
        neg_example.pop('label')
        pos_example=deepcopy(text)
        pos_example.pop('label')
#        neg_example.update({"label":0})
#        pos_example.update({"label":1})
        neg_example.update({"label":answer[choice]})
        pos_example.update({"label":answer[choice]})
        ...
        assert choice!=len(label_list), print("Make sure there are only two possible answers!")
        # reconvert to dataset format but with fake/candidate labels to create the contrast pair
        #neg_example = {self.text_key: text, "label": 0}
        #pos_example = {self.text_key: text, "label": 2}
        ...
        ans=', '.join(self.prompt.get_answer_choices_list(data))
        neg_prompt[0]=neg_prompt[0].replace("What label best describes","Among the following labels: {}, does label '{}' best describe".format(ans,neg_prompt[1]))
        #neg_prompt[0]=neg_prompt[0].replace("what category does the paragraph belong to","Does the paragraph belong to category '{}'".format(neg_prompt[1]))
    
        neg_prompt[1]='No, it does not.'
        pos_prompt[0]=pos_prompt[0].replace("What label best describes","Among the following labels: {}, does label '{}' best describe".format(ans,pos_prompt[1]))
        #pos_prompt[0]=pos_prompt[0].replace("what category does the paragraph belong to","Does the paragraph belong to category '{}'".format(pos_prompt[1]))
        
        pos_prompt[1]='Yes, it does.'
        # tokenize
        ...
        if self.use_decoder and self.model_type == "encoder_decoder":
            assert (abs(neg_ids["decoder_input_ids"] - pos_ids["decoder_input_ids"])).sum() != 0, print("The decoder_input_ids for the contrast pairs are the same!", neg_ids, pos_ids)
        else:
            assert (abs(neg_ids["input_ids"] - pos_ids["input_ids"])).sum() != 0, print("The input_ids for the contrast pairs are the same!", neg_prompt, pos_prompt)

        # return the tokenized inputs, the text prompts, and the true label
        return neg_ids, pos_ids, neg_prompt, pos_prompt, choice
...
class Zeroshot(Dataset):
    ...
    def encode(self, nl_prompt):
        ...
        # tokenize the question and answer (depending upon the model type and whether self.use_decoder is True)
        if self.model_type == "encoder_decoder":
            input_ids = self.get_encoder_decoder_input_ids(question)
        elif self.model_type == "encoder":
            input_ids = self.get_encoder_input_ids(question)
        else:
            input_ids = self.get_decoder_input_ids(question)
    ...
    def get_decoder_input_ids(self, question):
        """
        Format the input ids for encoder-only models.
        This is the same as get_encoder_input_ids except that we add the EOS token at the end of the input (which apparently can matter)
        """
    ...
    def __getitem__(self, index):
        ...
        wrong_answer=np.random.choice(lst)
        answer=np.array([wrong_answer,true_answer])
        example=deepcopy(text)
        example.pop('label')
        example.update({"label":true_answer})
        # get the possible labels
        # (for simplicity assume the binary case for contrast pairs)

        #assert len(label_list) == 2, print("Make sure there are only two possible answers! Actual number of answers:", label_list)
        assert choice!=2, print("Make sure there are only two possible answers!")

        # construct contrast pairs by answering the prompt with the two different possible labels
        # (for example, label 0 might be mapped to "no" and label 1 might be mapped to "yes")
        
        exm_prompt = self.prompt.apply(example)
        ans=', '.join(self.prompt.get_answer_choices_list(data))
        #exm_prompt[0] = exm_prompt[0]+" Please answer between <solution> tag within these choices: {}. ".format(ans)
        #exm_prompt[0] = "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n You are a helpful assistant<|eot_id|><|start_header_id|>user<|end_header_id|>\n" + exm_prompt[0] + "?<|eot_id|><|start_header_id|>assistant<|end_header_id|>"
        exm_prompt[0] = "You are helpful AI assistant. User: " + exm_prompt[0] +" Assistant: "
        
        # tokenize
        exm_ids = self.encode(exm_prompt[0])

    
        # return the tokenized inputs, the text prompts, and the true label
        return exm_ids, true_answer
    
def get_dataloader(dataset_name, split, prompt_name, tokenizer, prompt_idx, batch_size=16, num_examples=1000,
                   model_type="encoder_decoder", use_decoder=False, device="cuda",text_key='text',label_key='label', pin_memory=True, num_workers=0):
    """
    ...
    #id=np.array(np.where(np.array(raw_dataset['label'])!=1)).squeeze().tolist()
    #raw_dataset=raw_dataset.select(id)
    # load all the prompts for that dataset
    all_prompts = DatasetTemplates(*prompt_name)
    ...
    contrast_dataset = ContrastDataset(raw_dataset, tokenizer, all_prompts, prompt_idx, 
                                       ...
                                       device=device,text_key=text_key,label_key=label_key)
    #question_dataset = Zeroshot(raw_dataset, tokenizer, all_prompts, prompt_idx, 
    #                                   model_type=model_type, use_decoder=use_decoder, 
    #                                   device=device,text_key=text_key,label_key=label_key)
...
def get_individual_hidden_states(model, batch_ids, layer=None, all_layers=True, token_idx=-1, model_type="encoder_decoder", use_decoder=False):
    ...
    if token_idx == 0:
        ...
        final_hs = hs[torch.arange(hs.size(0)), (first_mask_loc+token_idx).detach().cpu()] # (bs, dim, num_layers)
...
def get_individual_output(model, batch_ids, layer=None, all_layers=True, token_idx=-1, model_type="encoder_decoder", use_decoder=False):
    """
    Given a model and a batch of tokenized examples, returns the hidden states for either 
    a specified layer (if layer is a number) or for all layers (if all_layers is True).
    ...
    with torch.no_grad():
        batch_ids = batch_ids.to(model.device)
        #output=model(**batch_ids, output_hidden_states=True)
        output = model.generate(**batch_ids,max_length=520)

    
    return output
def get_all_output(model, tokenizer,dataloader, layer=None, all_layers=True, token_idx=-1, model_type="encoder_decoder", use_decoder=False):
    """
    Given a model, a tokenizer, and a dataloader, returns the hidden states (corresponding to a given position index) in all layers for all examples in the dataloader,
    along with the average log probs corresponding to the answer tokens
    ...
    for batch in tqdm(dataloader):
        ...
        exm_ids,gt_label = batch

        exm_hs = get_individual_output(model,exm_ids, layer=layer, all_layers=all_layers, token_idx=token_idx, 
                                              model_type=model_type, use_decoder=use_decoder)

        #if dataloader.batch_size == 1:
        #    exm_hs = exm_hs.unsqueeze(0)
        decoded_output = tokenizer.decode(exm_hs[0], skip_special_tokens=True)
